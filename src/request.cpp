...
long requestLine (long request_line_cursor, std::ifstream &file) {
    ...
    // sprawdz czy za HTTP-VERSION jest CRLF
    ch = file.get();
    if (ch == CR) {
        // Sprawdzanie requestLine zakonczone powodzeniem
        ch = file.get();
        if (ch == LF) {
            return http_version_cursor+2;
        } else {
            return NOT_FOUND;
        }
...
long requestHeader(long request_header_cursor, std::ifstream &file) {
    ...
    if (field_name.compare("Host") == 0) {
        ...
        if (ch == LF) {
            report(HOST, host);
            return request_header_cursor + j + 2;
        } else {
            return NOT_FOUND;
        }
    } else {
        ...
        ch = file.get();
        if (ch == LF) {
            return request_header_cursor + j + 1;
        } else {
            return NOT_FOUND;
        }
    }
